...
public interface PersonsRepository extends JpaRepository<Person, Long>, JpaSpecificationExecutor<Person> {
    ...
    @Query(value = "SELECT new socialnet.api.response.RegionStatisticRs(p.city, COUNT(p.city)) FROM Person AS p " +
            "WHERE p.city IS NOT NULL " +
            "GROUP BY p.city")
    List<RegionStatisticRs> getCityWithUsersCount();

    @Query(value = "SELECT new socialnet.api.response.RegionStatisticRs(p.country, COUNT(p.country)) FROM Person AS p " +
